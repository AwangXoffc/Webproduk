document.addEventListener("DOMContentLoaded", () => {
        ...
        storeConfig.categories.forEach((category) => {
            ...
            card.innerHTML = `
                <div class="category-img-wrapper">
                    <img class="category-img" src="${category.image}" alt="${category.name}" loading="lazy" decoding="async">
                </div>
    ...
    let cart = JSON.parse(localStorage.getItem('awangOfficialCart')) || [];
    ...
    function saveCart() {
        localStorage.setItem('awangOfficialCart', JSON.stringify(cart));
        updateCartBadge();
    }
    ...
    if (ownerContactBtn) {
        ownerContactBtn.href = `https://wa.me/${storeConfig.whatsappNumber}?text=${encodeURIComponent("Halo " + storeConfig.ownerProfile.name + ", saya ingin bertanya seputar Awang OfficiaL.")}`;
    }
            ...
            if (role === "user") {
                ...
                msgDiv.innerHTML = `
                    <div style="display: flex; gap: 10px; align-items: flex-end; flex-direction: row-reverse;">
                        <div class="msg-bubble" style="background: linear-gradient(135deg, #343a40 0%, #121212 100%); color: #ffffff; padding: 14px 18px; border-radius: 20px; border-bottom-right-radius: 4px; box-shadow: 0 4px 15px rgba(52, 58, 64, 0.2); font-size: 14px; line-height: 1.6;">
                            ${formattedText}
                        </div>
                    </div>
                `;
                ...
                msgDiv.innerHTML = `
                    <div style="display: flex; gap: 10px; align-items: flex-end;">
                        <div style="width: 30px; height: 30px; background: #343a40; color: white; border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 12px; flex-shrink: 0; box-shadow: 0 2px 5px rgba(52, 58, 64, 0.2);">
                            <i class="fas fa-headset"></i>
                        </div>
                        <div class="msg-bubble" style="background: #fafafa; color: #2d3748; padding: 14px 18px; border-radius: 20px; border-bottom-left-radius: 4px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); font-size: 14px; line-height: 1.6; border: 1px solid #edf2f7;">
                            ${formattedText}
                        </div>
                    </div>
                `;
